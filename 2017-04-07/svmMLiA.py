...
def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup = ('lin', 0)):
	oS = optStruct(np.mat(dataMatIn), np.mat(classLabels).transpose(), C, toler, kTup)			# 创建实例
	iter = 0
...
def testRbf(k1 = 0.2):
	...
	labelSV = labelMat[svInd]								# 支持向量标签矩阵
	print "there are %d Support Vectors" % np.shape(sVs)[0]	
	m, n = np.shape(dataMat)
	errorCount = 0
	for i in range(m):										# 遍历每一个数据点
		kernelEval = kernelTrans(sVs, dataMat[i, :], ('rbf', k1))			# 计算核函数的值
		predict = kernelEval.T * np.multiply(labelSV, alphas[svInd]) + b 	# 对数据点进行预测
		if np.sign(predict) != np.sign(labelArr[i]): errorCount += 1		# 将预测结果与标签符号进行比较，相同则为预测正确
	print "the training error rate is: %f" % (float(errorCount) / m)		# 计算错误率
...
if __name__ == '__main__':
	...
	# print np.mat(dataArr)[93] * np.mat(ws) + b
	testRbf()
